...
export default function Primary() {
  ...
  const onSubmit = async (e) => {
    ...
    if (username !== "" && password !== "" && conPassword !== "" && name !== "" && (password === conPassword)) {
      try {
        var res = await signUpWithName(username, password, name);
        console.log(res);
            ...
            <Box component="form" sx={{ mt: 3, color: "white" }}>
              <Grid container spacing={2} sx={{
                ...
                <Grid item xs={12} sx={{ display: "grid", alignContent: "center" }} >
                  ...
                  />
                  {nameError !== "" && (
                    <Grid item xs={12} sx={{ color: "error.main", justifySelf: "left" }}>
                      {" "}
                      {nameError}{" "}
                    </Grid>
                  )}
              ...
              <Grid item>
                Existing account?
                <Link to="/login" variant="body2" style={{ marginLeft: '4px', color: "#5454d4" }}>
                  Login
                </Link>
              </Grid>
            </Box>
